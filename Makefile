...
COMMON_LIBS := -llinalg -lpthread -lrt 
WAYLAND_LIBS := -lwayland-egl -lwayland-server -lwayland-cursor -lwayland-client -lEGL -lGLESv2
IMG_LIBS := -ljpeg -lpng 
...
COMMON_SOURCES := $(SRCDIR)/keyboard.cpp \
	...
	$(SRCDIR)/heightmap.cpp \
	$(SRCDIR)/image_loader.cpp
...
DEDICATED_SERVER_DEPS := $(NETOBJS) $(OBJDIR)/keyboard.o $(OBJDIR)/common.o $(OBJDIR)/timer.o $(OBJDIR)/heightmap.o $(OBJDIR)/image_loader.o
...
kar: $(OBJECTS) $(NETOBJS) $(ALL_SOURCES) $(SRCDIR)/main.cpp
	$(CXX) $(OBJECTS) $(NETOBJS) $(SRCDIR)/main.cpp -o $@ $(INCLUDES) $(COMMON_LIBS) $(WAYLAND_LIBS) $(IMG_LIBS)

dedicated_server: $(DEDICATED_SERVER_DEPS) $(ALL_SOURCES) $(SRCDIR)/net/dedicated_server.cpp
	$(CXX) $(DEDICATED_SERVER_DEPS) $(SRCDIR)/net/dedicated_server.cpp -o $@ $(INCLUDES) $(COMMON_LIBS) $(IMG_LIBS)
