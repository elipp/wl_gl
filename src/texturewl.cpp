...
static int loadPNG(const std::string &filename, unsigned char **out_buffer, unified_header_data *out_header);
static int load_pixels(const std::string& filename, unsigned char **pixel_buffer, unified_header_data *img_info);
...
static int loadJPEG(const std::string &filename, unsigned char **out_buffer, unified_header_data *out_header) {
	...
	out_header->bpp = 8;	// safe assumption
	out_header->num_channels = cinfo.output_components;

	/* JSAMPLEs per row in output buffer */
	row_stride = cinfo.output_width * cinfo.output_components;
//	long total_bytes = (cinfo.output_width*cinfo.output_components)*(cinfo.output_height*cinfo.output_components);
...
static int load_pixels(const std::string& filename, unsigned char **pixel_buf, unified_header_data *img_info) {
...
static int loadPNG(const std::string &filename, unsigned char **out_buffer, unified_header_data *out_header) {
	...
	unified_header_data hdr;
	hdr.width = png_get_image_width(png_ptr, info_ptr);
	hdr.height = png_get_image_height(png_ptr, info_ptr);
	hdr.bpp = png_get_bit_depth(png_ptr, info_ptr);
	hdr.num_channels = png_get_channels(png_ptr, info_ptr);

	long total_bytes = hdr.height * bytes_per_row;
...
//	int number_of_passes = png_set_interlace_handling(png_ptr);
	...
	*out_buffer = pixel_buffer; 
	...
	png_byte **row_pointers = (png_byte**)malloc(sizeof(png_byte*) * hdr.height);

	// assign row pointers individually as offsets into the contiguous opengl-compatible uchar buffer
	for (int i = 0; i < hdr.height; ++i) row_pointers[(hdr.height-1)-i] = pixel_buffer + i*bytes_per_row;	
